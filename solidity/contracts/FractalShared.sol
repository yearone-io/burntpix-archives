...
contract FractalShared is IERC165, IERC725Y {
    ...
    function getArchiveStats() external view returns (ArchiveData memory) {
        ArchiveData memory archive = ArchiveData({
            canvas: canvas,
    ...
    function setArchivePixels(ArchiveData calldata archive) external {
        // require(msg.sender == registry);
        // write canvas pixels
        canvas.pixels = archive.canvas.pixels;
    }
