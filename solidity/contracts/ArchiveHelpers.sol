...
contract ArchiveHelpers {
    ...
    function alterImageBasedOnLevel(
        bytes memory originalBytes,
        uint256 imageLevel,
        uint256 highestLevel
    ) internal pure returns (bytes memory) {
        bytes
            memory oldStartBytes = hex"";
        bytes memory newStartBytes = abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 385 385"><defs><linearGradient id="gradient1" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" style="stop-color:red; stop-opacity:calc(',
            uintToString(imageLevel),
            "/",
            uintToString(highestLevel),
            ')"/><stop offset="100%" style="stop-color:blue; stop-opacity:calc(',
            uintToString(imageLevel),
            "/",
            uintToString(highestLevel),
            ')"/></linearGradient></defs>',
        ...
        bytes memory result = new bytes(
            newStartBytes.length + originalBytes.length - oldStartBytes.length
        );
        ...
        for (uint i = oldStartBytes.length; i < originalBytes.length; i++) {
            result[
                newStartBytes.length + i - oldStartBytes.length
            ] = originalBytes[i];
        }
    ...
    function getVerifiableURI(
        bytes memory rawMetadata
    ) internal pure returns (bytes memory) {
        return
            bytes.concat(
    ...
    function getCoreMetadata(
        ...
        return
            abi.encodePacked(
                ...
                '"description": "We burn together, we rise together. A community built archive of BurntPix Fractal ',
                addressToString(burntPicFractal),
                '",',
                '"images": [[{"height": 768, "width": 768, "url": "',
                encodedImage,
                '", "verification": {"method": "keccak256(bytes)", "data": "',
                toHexString(keccak256(image), 32),
                '"}}]],',
    ...
    function generateCollectionMetadata(
        ...
        bytes memory image = alterImageBasedOnLevel(
            IFractal(fractalClone).getData(keccak256("image")),
            1,
            1
        );
    ...
    function generateArchiveMetadata(
        ...
        string memory children = string(
            abi.encodePacked(
                ',"attributes": [{"key": "Level", "type": "number", "value": ',
                uintToString(archive.level),
                '}, {"key": "Block Number", "type": "number", "value": ',
                uintToString(archive.blockNumber),
                '}, {"key": "Iterations", "type": "number", "value": ',
                uintToString(archive.iterations),
                '}, {"key": "Creator", "type": "string", "value": "',
                addressToString(archive.creator),
                '"}]'
            )
        );
