...
contract Fractal is IERC165, IERC725Y {
    ...
    constructor(address _codehub, uint256 _seed, uint256 _width, uint256 _height) {
        registry = msg.sender;
        codehub  = _codehub;

        (bool ok, ) = CodeHub(codehub).attractor().delegatecall(
            abi.encodeWithSignature("init(uint256,uint256,uint256)", _seed, _width, _height)
        );
        require(ok);
    }

    // migrate switches the fractal collection registry to a new one. The purpose
    // is to move the NFTs from a pre-release registry contract to a newer one if
    // they are specced and released.
    function migrate(address _registry) public {
