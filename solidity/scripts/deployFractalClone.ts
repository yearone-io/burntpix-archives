...
import FractalClone from "../artifacts/contracts/FractalClone.sol/FractalClone.json";
...
async function main() {
  ...
  const collectionName = "FractalClone";
  const symbol = "HOP";
  const contractOwner = UP_ADDR;
  const constructorArguments = [registry, codehub, burntpicId, maxArchiveSupply, collectionName, symbol, contractOwner];
  const FractalCloneFactory = new ethers.ContractFactory(
    FractalClone.abi,
    FractalClone.bytecode,
  );
  const onchainHouse = await FractalCloneFactory.connect(signer).deploy(
